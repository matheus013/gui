...
public class BDCodigoDAO implements InterfaceDAO<Codigo> {
    ...
    public void trocar(Codigo obj_code) {
        Connect conexao = new Connect();
        String sql = "UPDATE ico_code set  email=?, level=? WHERE code=?;";
        try {
            PreparedStatement pc = conexao.getConnection().prepareStatement(sql);
            pc.setString(1, obj_code.getIco_email());
            pc.setInt(2, obj_code.getIco_level());
            pc.setString(3, obj_code.getIco_code());
            pc.execute();
            conexao.getConnection().close();
        } catch (SQLException ex) {
            ex.printStackTrace();
